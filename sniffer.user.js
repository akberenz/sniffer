...
(async () => {
  //  SETUP  //
  var OPTIONS = {
    pinned: { key: "pinned", value: await GM.getValue("pinned", false), text: "Keep Pinned" },
    alwaysAll: { key: "alwaysAll", value: await GM.getValue("alwaysAll", false), text: "Always Show All" }
  };
  ...
  var lookFor = {
    ...
    genetic: function(finds, postId, string) {
      ...
      var seq = string.match(/\b((?:[acguACGU]{3}|\d+)(?:\s+|\b|$)){3,}/g);
      if (seq) {
        for(var i=0; i<seq.length; i++) {
          var at = seq[i].trim();
          if (at.toUpperCase() !== "AAA" && at.toUpperCase() !== "GAA" && at.replace(/\d+/g,'').trim()) {
            ...
            for(var l=0; l<letters.length; l++) {
              var mapped = GENETIC_MAP[letters[l].toUpperCase()];

              if (mapped) {
                // first letter in each trio decides case
                var first = letters[l][0];
  ...
  var visualize = {
    style: function() {
      GM.addStyle("div.comment__secrets{ display: inline-block; cursor: pointer; margin-right: 6px; }" +
      ...
      if ($("body").css("background-color") !== "rgb(149, 164, 192)") {
        ...
        GM.addStyle(".suspicion_none{ color: #2E2E2C; }" +
  ...
  var init = {
    ...
    settingBlock: function(setting) {
      ...
      $content.on("click", async function() {
        $content.toggleClass("is-selected");

        setting.value = !setting.value;
        await GM.setValue(setting.key, setting.value);
        init.options();
      });
